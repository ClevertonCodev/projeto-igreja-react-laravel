...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        try {
            ...
            if (!$user || !$user->ativo) {
                return response()->json(['error' => 'Usuário inativo ou inexistente'], Response::HTTP_NOT_FOUND);
            }

            $credentials = $request->only(['email', 'password']);
            if (!$token = auth('api')->attempt($credentials)) {
                return response()->json(['error' => 'Senha ou email inválido'], Response::HTTP_UNAUTHORIZED);
            }
    ...
    public function validadeEmailAndCPF(Request $request)
    {
        try {
    ...
    {
        if ($request->user()->hasVerifiedEmail()) {
            return view('index')->with('emailVerified', true);
        }
